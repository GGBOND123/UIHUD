...
//
///////////////////////////////////////////////////////////
//NGUI渲染UI的方式，实际上就是用Unity的UnityEngine.MeshRenderer来生成渲染图元。
//一个MeshRenderer，经过相机绘制到屏幕上，实际上就是一次Draw Call。NGUI的渲染实体UIDrawCall里，就包含MeshRenderer、MeshFilter和Mesh本身的生成过程，
////所以一个UIDrawCall实例就意味着一次Draw Call，这也是NGUI的设计原则。
...
class HUDVertex
{
    ...
    public static HUDVertex QueryVertex()
    {
        ...
        //如果链表一直新增节点，s_InvalidList一直为null
        if (p == null)
        {
            p = new HUDVertex();
            p.m_nID = ++s_nVertexID;
